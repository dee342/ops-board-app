...
public class TestCommandUnAssignEquipToPartialTask extends TestBaseCommands{

	private static final Logger logger = LoggerFactory.getLogger(TestCommandUnAssignEquipToPartialTask.class);
	@Before
	public void setUp(){	
	...
	//	System.out.println("task container "+taskContainer.getLocationShifts().contains(shiftCategories.contains(subcategoryTasks.contains(subcategoryTask.getTasks().get(0).getId()))));
		...
		EquipmentAssignment equimentAssgn1 = createEquipmentAssignment(boardEquipment1, 1L);
		EquipmentAssignment equimentAssgn2 = createEquipmentAssignment(boardEquipment1, 2L);
...
/*TODO - Fix
 * 	//unassign a equip is from start truck of a partial taskassigned it unassign from the 2nd tasks in same catogery also and assert
 *
	@Test
	public void testUnAssignEquipmentToPartialTask() throws Exception {
		
		Map<String, Object> content = new HashMap<String, Object>();
		content.put("taskId", "1");
		content.put("equipmentId", EQUIP_ID1);
		
		//TODO create task in same catogery
		CommandUnassignEquipmentFromPartialTask commandUnAssignEquipmentToPartialTask = new CommandUnassignEquipmentFromPartialTask(board.getId(), content);
		commandUnAssignEquipmentToPartialTask.setLocation(location);
		commandUnAssignEquipmentToPartialTask.setSystemDateTime(now);
		commandUnAssignEquipmentToPartialTask.execute(board);
		
		Assert.assertEquals(EQUIP_ID1, commandUnAssignEquipmentToPartialTask.getEquipmentId());
		Assert.assertTrue(commandUnAssignEquipmentToPartialTask.getAssignments().size() >= 1);	
	}
	*/
	
		//
		@Test
		public void testUnAssignEquipmentToPartialTask1() throws Exception {	
